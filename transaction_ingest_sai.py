...
class SAIQueryDemonstrator:
    """Demonstrates SAI query capabilities with performance tracking - HIGH CARDINALITY ONLY"""
    ...
    def prepare_query_statements(self):
        ...
        self.queries = {
            ...
            'by_timestamp': self.session.prepare(
                "SELECT * FROM transactions WHERE timestamp > ? LIMIT ? ALLOW FILTERING"
            ),
        }
        logger.info("SAI query statements prepared (high cardinality only)")
    ...
    def query_by_merchant(self, merchant_id: str = None, limit: int = 10) -> QueryResult:
        """Query transactions by merchant using SAI index - HIGH CARDINALITY"""
    ...
    def query_by_country(self, country: str = "US", limit: int = 10) -> QueryResult:
        """Query transactions by country using SAI - MEDIUM CARDINALITY"""
        result = self.execute_timed_query('by_country', 'country_lookup', country, limit)
        self.query_results.append(result)
        return result
    
    def query_by_recent_timestamp(self, minutes_back: int = 60, limit: int = 10) -> QueryResult:
        """Query recent transactions - HIGH CARDINALITY"""
        cutoff = datetime.now(timezone.utc) - timedelta(minutes=minutes_back)
        result = self.execute_timed_query('by_timestamp', 'timestamp_lookup', cutoff, limit)
        self.query_results.append(result)
        return result
    ...
    async def run_continuous_queries(self, duration_seconds: int = 30):
        """Run continuous queries for performance testing - HIGH CARDINALITY ONLY"""
        logger.info(f"Starting continuous SAI queries for {duration_seconds}s (high cardinality)...")
        end_time = time.time() + duration_seconds
        
        # Only high cardinality queries with weighted distribution
        query_functions = [
            (lambda: self.query_by_merchant(), 50),           # 50% - Most selective
            (lambda: self.query_high_risk(), 30),             # 30% - Range queries
            (lambda: self.query_by_country(), 15),            # 15% - Medium cardinality
            (lambda: self.query_by_recent_timestamp(), 5)     # 5% - Time-based
        ]
        
        # Create weighted list
        weighted_queries = []
        for func, weight in query_functions:
            weighted_queries.extend([func] * weight)
        
        while time.time() < end_time:
            # Execute random weighted query
            query_func = random.choice(weighted_queries)
            query_func()
            await asyncio.sleep(0.01)  # Small delay between queries
        
        logger.info(f"Completed {len(self.query_results)} queries (high cardinality only)")
...
async def main():
    ...
    try:
        ...
        """
        print("\n" + "="*60)
        print("SAI BENEFITS DEMONSTRATED (HIGH CARDINALITY)")
        print("="*60)
        print("✓ 4 high-cardinality indexes (merchant_id, risk_score, country, timestamp)")
        print("✓ Automatic index updates on write")
        print("✓ Inequality queries (risk_score > 0.8)")
        print("✓ Time-range queries (timestamp > cutoff)")
        print("✓ Lower storage overhead (no low-cardinality indexes)")
        print("✓ Optimized query performance on selective columns")
#        print("\n⚠️  Low cardinality columns NOT indexed:")
#        print("   - status (3-4 values) → use partitioned tables")
#        print("   - gateway (3 values) → use partitioned tables")
#        print("   - currency (5-10 values) → filter in application")
